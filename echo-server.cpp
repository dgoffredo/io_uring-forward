...
int main() {
  ...
  const int rc = [&]() {
    ...
    POSIX_REQUIRE(
        bind(listen1fd, (sockaddr *)&serv_addr, sizeof(serv_addr)));
    ...
    POSIX_REQUIRE(
        bind(listen2fd, (sockaddr *)&serv_addr, sizeof(serv_addr)));
    POSIX_REQUIRE(listen(listen2fd, 1));

    std::cout << "Waiting for observer client to connect on port " << observer_port << ".\n";
  ...
  for (const int fd : {conn1fd, listen1fd, pipe1fds[0], pipe1fds[1], conn2fd, listen2fd, pipe2fds[0], pipe2fds[1]}) {
    if (fd >= 0) {
      close(fd);
    }
  }
