...
int main() {
  ...
  const int rc = [&]() {
    ...
    POSIX_REQUIRE(pipe(pipefds));
    POSIX_REQUIRE(listenfd = socket(AF_INET, SOCK_STREAM, 0));
    ...
    POSIX_REQUIRE(listen(listenfd, 10));

    POSIX_REQUIRE(connfd = accept(listenfd, NULL, NULL));
