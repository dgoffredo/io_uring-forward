...
int server_splice(int bufsize, io_uring &ring, int conn1fd,
                  int (&pipe1fds)[2]) {
  ...
  for (;;) {
    ...
    for (int i = 0; i < 2; i++) {
      ...
      if (result < io_ctx.bytes_desired) {
        if (io_ctx.from_fd == conn1fd) {
          ++metrics.short_reads;
        }
      }
      if (io_ctx.from_fd == conn1fd && result == 0) {
...
int main(int argc, char *argv[]) {
  ...
  const int rc = [&]() {
    ...
    switch (server_mode) {
      ...
      case SPLICE:
        return server_splice(bufsize, ring, conn1fd, pipe1fds);
      default:
        std::unreachable();
    }
  }();
